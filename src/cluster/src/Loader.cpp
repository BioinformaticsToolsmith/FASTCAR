...
template<class T>
void Loader<T>::preload(int tid, std::string prefix)
{
...
//	cout << "mag: " << ((DivergencePoint<T>*)p)->getPseudoMagnitude() << std::endl;
	...
	p->set_effective_size(chrom->getEffectiveSize());
	if (set_seq) {
...
template<class T>
void Loader<T>::load_next(int tid, std::vector<Point<T>*>& points)
{
	points.reserve(cache_list.at(tid).size());
